...
type ForumViewParams = {
    forumPostID?: string; // <-- make it optional to support both /forum and /forum/:forumPostID
};

type ForumViewProps = RouteComponentProps<ForumViewParams> & {
...
export function ForumView({ onError, match, history }: ForumViewProps) {
    ...
    // forumPostID will be undefined for /forum, so default to empty string
    ...
    function handleNextButton() {
        dispatch({
            _type: 'CHANGE_CURRENT_PAGE',
            currentPage: totalPages - 1 === currentPage ? currentPage : currentPage + 1
        });
    }
                        ...
                        >
                            ...
                            {languagesData &&
                                languagesData.map((languages, index) =>
                                    <option key={index} value={languages.languageID}>
            ...
            <Container>
                ...
                <Row>
                    <Col className='d-flex justify-content-center mt-5'>
                        <PaginationButtons
                            firstPage={handleFirstPageButton}
                            previousPage={handlePreviousButton}
                            nextPage={handleNextButton}
                            lastPage={handleLastPageButton}
                        />
                    </Col>
                </Row>
            </Container>
